...
//    }
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onBackRoot) name:@"signOut" object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onHide) name:k_hideTab object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onShow) name:k_showTab object:nil];
...
- (void)setupTabBarForPosition:(NGTabBarPosition)position {
    ...
    switch (position) {
        ...
        case NGTabBarPositionBottom: {
            frame = CGRectMake(0.f, self.view.bounds.size.height - dimension - 20, self.view.bounds.size.width, dimension + 20);
...
- (void)handleItemPressed:(id)sender
{
    self.curIndex = [self.tabBarItems indexOfObject:sender];
    [self processItemPressed];
}

- (void) processItemPressed
{
    BOOL shouldSelect = [self.delegate tabBarController:self shouldSelectViewController:[self.viewControllers objectAtIndex:self.curIndex] atIndex:self.curIndex];
   
    if (shouldSelect) {
        if (self.curIndex != self.selectedIndex) {
            self.selectedIndex = self.curIndex;
        } else {
...
#pragma marl -
#pragma mark - TabBarClick Delegate //chagned by Zong
